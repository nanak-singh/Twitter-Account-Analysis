...
# Collections.Counter() = implements specialized container datatypes providing alternatives to Python’s general purpose built-in containers, dict, list, set, and tuple.
...
def print_charts(dataset, title, weekday=False):
    ...
    for key in keys:
        if (dataset[key] >= median * 1.33):
            displayed_key = "%s (+)" % (int_to_weekday(key) if weekday else key)
    ...
    thresholds = {
    #according to average user activity I have customised the chart
...
def main():
	#usage of tweepy
	#authenticaltion of python code with my API token
    ...
    #Time Zone of User
    user_data['user_time_zone'] = user_info.time_zone
    #offset of user time with respect to GMT
    user_data['user_utc_offset'] = user_info.utc_offset

    if user_info.utc_offset is None:
        print("[!] Can't get specific timezone for this user")
        user_data['user_utc_offset_note'] = "Can't get specific timezone for this user"

    if args.utc_offset:
        print("[!] Applying timezone offset %d (--utc-offset)" % args.utc_offset)
        user_data['user_utc_offset_set'] = "Applying timezone offset %d (--utc-offset)" % args.utc_offset

    # Number of Tweets
    print("[+] statuses_count : %s" % user_info.statuses_count)
    user_data['status_count'] = user_info.statuses_count

    # Will retreive all Tweets from account (or max limit of API)
    ...
    user_data["activity_hourly"] = activity_hourly
    user_data["activity_weekly"] = activity_weekly

    print("[+] Detected languages (top 5)")
    print_stats(detected_langs)
    user_data["top_languages"] = detected_langs

    print("[+] Detected sources (top 10)")
    print_stats(detected_sources, top=10)
    user_data["top_sources"] = detected_sources
    ...
    if len(detected_places) != 0:
        ...
        user_data["top_places"] = detected_places

    print("[+] Top 10 hashtags")
    print_stats(detected_hashtags, top=10)
    user_data["top_hashtags"] = detected_hashtags
    ...
    if not args.no_retweets:
        ...
        user_data["top_retweeted_users"] = retweeted_users_names
    ...
    user_data["top_mentioned_users"] = mentioned_users_names

    print("[+] Most referenced domains (from URLs)")
    print_stats(detected_domains, top=6)
    user_data["top_referenced_domains"] = detected_domains
